...
async function fetchKugouVipInfoWeb(cookie, auth) {
  try {
    const text = await requestText('https://vip.kugou.com/recharge/roleinfo', {
      headers: {
        ...KUGOU_HEADERS,
        Host: 'vip.kugou.com',
        Accept: '*/*',
    ...
    if (resp && resp.errno === 0 && resp.error_code === 0 && resp.vipRemains > 0 && resp.isExpiredMember === 0) {
      return {
        status: 1,
        data: {
          isVip: true,
          vipType: 1,
          vip: 1,
          vipRemains: resp.vipRemains,
          vipEndTime: resp.vipEndTime || 0,
        },
      };
    }
    return { status: 0 };
...
async function fetchKugouVipInfo(cookie, auth) {
  ...
  return kugouVipCache.wrap(cacheKey, (value) => {
    ...
    const attempts = [
      ...
      () => fetchKugouVipInfoWeb(cookie, auth),
    ];
...
module.exports = {
  ...
  _test: {
    ...
    extractKugouAuth,
    pickKugouQualityHashMatch,
    kugouEnrichQualityHashes,
    kugouDurationCloseMs,
    kugouQualityHashCacheKey,
  },
};
